...
genres_ranked = [a.text.strip().replace(' ', '-') for a in target_widget.select("div.table-cell.primary a")]
...
for num_link in range(len(links)):
    ...
    imdb_id_strs = [a["href"].split('/')[2] for a in soup.select("h3.lister-item-header a")]
    titles = [a.text for a in soup.select("h3.lister-item-header a")]
    num_voted_users = [span["data-value"] for span in soup.select("p.sort-num_votes-visible > span:nth-of-type(2)")]
    release_year_raw = [y.text for y in soup.select("span.lister-item-year.text-muted.unbold")]
    ...
    imdb_ratings = [float(r.text) for r in soup.select("div.inline-block.ratings-imdb-rating strong")]
    genres = [g.text.strip("\n").replace(' ','') for g in soup.select("span.genre")]
    descriptions = [child.find_all('p')[1].text.strip("\n") for child in movie_info_blocks]
    ...
    for i in range(len(titles)):
        writer.writerow([imdb_id_strs[i],
                         titles[i],
                         release_years[i],
                         certificates[i],
                         runtime_min[i],
