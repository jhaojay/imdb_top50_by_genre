            ...
            for row in content:
                ...
                for person in people:
                    ...
                    if "NULL" not in person:
                        ...
                        self.db.query(
                            insert_script, (person_id, imdb_person_id, full_name)
                        )
    ...
    def load_movie_info(self):
        ...
        with open(self.csv_file_path, newline="", encoding="utf-8") as cf:
            ...
            for row in content:
                ...
                if (
                        self.db.status_message()[-1] == "1"
                ):  # increases id only when movie is unique
                    movie_id += 1
        ...
        with open(self.csv_file_path, newline="", encoding="utf-8") as cf:
            ...
            for row in content:
                ...
                for genre in genres_for_each_movie:
                    ...
                    genre_id_fetched = self.db.query(
                        fetch_script, (genre.strip(),), fetch=True
                    )
                    ...
                    # fetch movie_info.id
                    fetch_script = (
                        """SELECT id FROM movie_info WHERE imdb_movie_id=%s """
                    )
                    movie_id_fetched = self.db.query(
                        fetch_script, (row["imdb_movie_id"],), fetch=True
                    )
    ...
    def load_movie_ranking_by_genre(self):
        ...
        with open(self.csv_file_path, newline="", encoding="utf-8") as cf:
            ...
            for row in content:
                ...
                fetch_script = """SELECT id FROM genre WHERE genre_type=%s """
                genre_id_fetched = self.db.query(
                    fetch_script, (row["ranked_genre"].strip(),), fetch=True
                )
                ...
                insert_script = """INSERT INTO 
                                    ...
                                    ON CONFLICT DO NOTHING"""
                self.db.query(
                    insert_script, (movie_id, ranked_genre_id, row["ranking_by_genre"])
                )
        ...
        with open(self.csv_file_path, newline="", encoding="utf-8") as cf:
            ...
            for row in content:
                ...
                for dirct in row["director"].split(","):
                    ...
                    person_id_fetched = self.db.query(
                        fetch_script, (imdb_person_id,), fetch=True
                    )
            ...
            for row in content:
                ...
                for cast in row["cast"].split(","):
                    if "NULL" not in cast:
                        ...
                        # fetch person_id
                        fetch_script = (
                            """SELECT id FROM person WHERE imdb_person_id=%s """
                        )
                        person_id_fetched = self.db.query(
                            fetch_script, (imdb_person_id,), fetch=True
                        )
